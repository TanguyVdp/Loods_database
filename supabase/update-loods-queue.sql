-- ============================================================
-- Wietloods — update: verwerkingswachtrij + klok-kalibratie (Boss menu)
...
--
-- LET OP: als je een oudere versie van dit bestand al had gerund (met een
-- "value"-kolom = ruwe voorraadwaarde), vervangt dit die door "offset_minutes"
-- = hoeveel de hele wachtrij-klok verschoven staat. Je vorige kalibratie
-- wordt daarmee gereset naar 0 (ongekalibreerd) — kalibreer gewoon opnieuw
-- via het Boss menu na het runnen van dit bestand.
-- ============================================================
...
alter table loods_baseline drop column if exists value;
alter table loods_baseline add column if not exists offset_minutes integer not null default 0;

insert into loods_baseline (id, offset_minutes, batch_size, batch_minutes, set_at) values (true, 0, 3, 3, now())
...
-- Boss-menu-acties (profiel resetten/verwijderen/herschikken). De boss vult
-- de werkelijke voorraad in; de website berekent zelf hoeveel minuten de
-- klok van de hele wachtrij moet verschuiven en stuurt enkel dat getal door.
