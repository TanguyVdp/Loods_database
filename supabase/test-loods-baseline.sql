-- ============================================================
-- Wietloods — TEST: kalibratiepunt + instelbare verwerkingssnelheid
-- Enkel nodig voor de test-wachtrij (test/index.html). Voer dit uit in de
-- Supabase SQL Editor als je de wachtrij-test wil proberen. Als je een
-- vorige versie van dit bestand al gerund had, mag je dit gewoon opnieuw
-- runnen — alles gebruikt "if not exists" / "add column if not exists" /
-- "create or replace", dus onschadelijk.
-- ============================================================
...
alter table loods_baseline add column if not exists batch_size integer not null default 3;
alter table loods_baseline add column if not exists batch_minutes integer not null default 3;
...
create or replace function fn_set_loods_baseline(p_value integer, p_batch_size integer, p_batch_minutes integer, p_set_by text)
...
grant execute on function fn_set_loods_baseline(integer, integer, integer, text) to anon, authenticated;
