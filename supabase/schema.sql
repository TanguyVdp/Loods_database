...
begin
  ...
  select * into v_user from users where id = p_user_id for update;
  if v_user.id is null then raise exception 'GEBRUIKER_NIET_GEVONDEN'; end if;
  v_tegoed := floor(v_user.total_ingelegd / 2.0) - v_user.opgehaald;
  ...
  insert into loods_log(type, user_id, user_name, amount, tegoed_after)
    values ('ophalen', v_user.id, v_user.name, p_amount, floor(v_user.total_ingelegd/2.0) - v_user.opgehaald)
    returning * into v_log;
  return v_log;
end; $$;
